const urlFoglio = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSB3hRuuuzUhGho2Wr_c0KBa8BYYMyASu6n0DrQ8q_o3GU3aw4_oC2vEvhuVbtmqR3Z3y0C5Zgh8Dji/pub?output=csv'; 
    ...
    // LIMITI MAPPA AGGIUNTI (maxBounds)
    var map = L.map('map', { 
    ...
    // Toggle logico dei badge automatici
    window.toggleAutoBadges = function() {
        const mapEl = document.getElementById('map');
        if (document.getElementById('auto-badge-toggle').checked) {
    ...
    // --- FILTRO CROMATICO AGGIORNATO CON VIOLA, CELESTE, ARANCIONE ---
    ...
    return colors.some(hex => {
        ...
        switch(targetColor) {
            case 'red':       return (r > 150 && g < 80 && b < 80);
            case 'maroon':    return (r > 80 && r < 160 && g < 50 && b < 60);
            case 'orange':    return (r > 200 && g > 100 && g < 190 && b < 80);
            case 'gold':      return (r > 170 && g > 140 && b < 100 && b > 30);
            case 'yellow':    return (r > 180 && g > 150 && b < 100);
            case 'lime':      return (g > 180 && r > 100 && b < 120);
            case 'green':     return (g > 100 && g < 190 && r < 120 && b < 120);
            case 'lightblue': return (b > 180 && g > 120 && r < 200);
            case 'blue':      return (b > 150 && r < 120 && g < 150);
            case 'navy':      return (b > 60 && b < 150 && r < 60 && g < 80);
            case 'purple':    return (r > 90 && b > 120 && g < 80);
            case 'pink':      return (r > 200 && g > 100 && b > 140);
            case 'brown':     return (r > 80 && r < 165 && g > 50 && g < 120 && b < 80);
            case 'white':     return (r > 215 && g > 215 && b > 215);
            case 'grey':      return (Math.abs(r-g) < 20 && Math.abs(r-b) < 20 && r > 70 && r < 190);
        ...
        const renderList = (limit) => {
            content.innerHTML = dati.slice(0, limit).map((d, i) => `
                <div class="ranking-item" onclick="vaiAClub('${d.nome.replace(/'/g, "\\'")}')">
                    ...
                    <img src="${d.logo_attuale}" width="20" height="20" style="object-fit:contain">
                    <span>${d.nome}</span>
    ...
    Papa.parse(urlFoglio, {
        ...
        complete: function(results) {
            ...
            // Centramento automatico sulla Nazione selezionata
            ...
            data.forEach((s, index) => {
                ...
                // Badge Automatici (con classe auto-badge per poterli nascondere con il toggle)
                ...
                if (altitudine > 2000) badgesHTML += '<div class="badge-icon badge-lc badge-tetto auto-badge" title="Tetto del Mondo">🏔️</div>';
                ...
                const localTimeStr = s.timezone ? new Intl.DateTimeFormat('it-IT', { timeZone: s.timezone, hour: '2-digit', minute: '2-digit' }).format(new Date()) : 'N.D.';
                const weatherId = `weather-${s.nome.replace(/\s/g, '')}`;
                    ...
                    <div class="popup-card">
                        ...
                        ${annivItems.length > 0 ? `
                            <div class="anniversario-box">
                                ...
                                ${annivItems.length > 3 ? `<div class="anniversario-grid hidden-logos" style="margin-top:8px;">${annivItems.slice(3).join('')}</div><button class="btn-espandi" onclick="toggleLoghi(this)">mostra altri ▼</button>` : ''}
                            </div>` : ''}
                        ${loghiItems.length > 0 ? `
                            <div class="storici-container">
                                <span class="box-label">Loghi Storici</span>
                                <div class="storico-grid">${loghiItems.slice(0,3).join('')}</div>
                                ${loghiItems.length > 3 ? `<div class="storico-grid hidden-logos" style="margin-top:8px;">${loghiItems.slice(3).join('')}</div><button class="btn-espandi" onclick="toggleLoghi(this)">mostra altri ▼</button>` : ''}
                            </div>` : ''}
            ...
            window.applicaFiltri = function() {
                ...
                visibiliAttualmente = allMarkers.filter(m => {
                    ...
                    const alt = parseInt(m.dati.altitudine) || 0;
